(ns execsecretary.actor
  "ExecutiveSupportActor — the ISCO-08 3343 community executive-support
  actor as a `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one executive-support operation
  ...
  resume after human sign-off. Modeled on cloud-itonami-isco-3313's
  accountingsupport.actor.
  ...
  The unconditional invariant: the ExecutiveSupportAdvisor can never
  directly commit a record the ExecutiveSupportGovernor refuses, and
  can never finalize a signature or financial/contractual commitment
  on the executive's behalf — every commit-record! call is gated
  behind `:decide`."
...
(defn build-graph
  "Build a compiled ExecutiveSupportActor graph. `store` implements
  `execsecretary.store/Store`. `advisor` implements
  `execsecretary.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:executive-id (:executive-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
